module.exports = {
  ...
  deploy: {
    development: {
      ...
      'post-deploy':
        'export NVM_DIR="$HOME/.nvm" && [ -s "$NVM_DIR/nvm.sh" ] && . "$NVM_DIR/nvm.sh" && cd /home/thangl-vietis/deploy-pm2/current && npm instal && npm run build && pm2 start ecosystem.config.js --env development', // Script chạy trên server sau khi deploy.
      /*       'pre-deploy':
